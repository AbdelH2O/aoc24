let () = 
  ...
  let index x y =
    try
      Array.get (Dynarray.get grid x) y
  ...
  let rec check x y direction ind =
    match ind with
    | 3 -> true
    | ind -> if (Array.get sequance ind) = (index x y) then (check (x + (fst direction)) (y + (snd direction)) direction (ind + 1)) else false
  in
  ...
  let check_x x y direction pos =
    if (check x y direction 0) && (check (x + (fst pos)) (y + (snd pos)) (matching_dir direction pos) 0) then 1 else 0
  in
