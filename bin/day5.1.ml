let () = 
  ...
  try
    while true do
      let rule = input_line inp |> String.split_on_char '|' |> List.filter (fun s -> s <> "") in
      if (List.length rule) = 0 then
        raise End_of_file
  ...
  let rec all_deps_printed deps exists visited =
    ...
    | [] -> true
    | dep :: other_deps -> (
      if (Hashtbl.mem exists dep) && (not (Hashtbl.mem visited dep)) then
        false
  ...
  let rec check pages exists visited =
    ...
    | page :: other_pages -> (
      ...
      (all_deps_printed (get_dependencies dependencies page) exists visited) && (check other_pages exists visited)
    )
  in
  ...
  let valid_order pages exists =
    ...
    List.iter (fun page ->
      ...
      List.iter (fun neighbor ->
        if (Hashtbl.mem exists neighbor) then (
          (add_dep dependents neighbor page);
          (Hashtbl.replace in_degree page ((Hashtbl.find_opt in_degree page |> Option.value ~default:0) + 1);
          if not (Hashtbl.mem in_degree neighbor) then Hashtbl.add in_degree neighbor 0))
      ) neighbors;
    ) pages;
  ...
  try
    while true do
      ...
      if not (check pages exists visited) then count := !count + (int_of_string (List.nth (valid_order pages exists) ((List.length pages)/2)))
    done;
  with End_of_file ->
