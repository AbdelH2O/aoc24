...
let is_invalid num polarity =
  abs(num) < 1 || abs(num) > 3 || ((num > 0) != polarity)

let rec check polarity elements = 
  match elements with
  | [] -> 1
  | x :: rest -> (
    if is_invalid x polarity then
      0
    else
      check polarity rest
  )
...
let rec get_polarity pos neg elements =
  ...
  | x :: rest -> if x > 0 then get_polarity (pos + 1) neg rest else (if x < 0 then get_polarity pos (neg + 1) rest else get_polarity pos neg rest)

let rec check_all polarity prev rest =
  match rest with
  | [] -> check polarity (diff prev (List.nth prev 0) true)
  | cur :: other -> if (check polarity (diff (prev @ other) (List.nth (prev @ other) 0) true)) == 1 then 1 else (check_all polarity (prev @ [cur]) other)
